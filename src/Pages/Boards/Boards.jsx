...
const Boards = () => {
  ...
  return (
    <div className="p-6 w-full min-h-screen bg-[#F3F4F6]">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">Your Boards</h1>
        <button
          onClick={() => setIsModalOpen(true)}
          className="px-2 md:px-4 py-2 bg-purple-900 text-white font-medium rounded-lg shadow hover:bg-purple-700 transition text-xs md:text-lg"
        >
          + Add Board
        </button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-6">
          {boardsData?.map((board) => (
            <div
              key={board._id}
              className="bg-white p-5 shadow-lg rounded-lg hover:scale-102 transform duration-300 transition"
            >
